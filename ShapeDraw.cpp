...
void DrawCircle(float_t radius, float_t x, float_t y, char8_t r, char8_t g, char8_t b, bool filled)
{
	...
	if (!filled)
	{
		...
		glPointSize(1.0);
		glBegin(GL_POINTS);
		auto radiusMax = radius + 2.0f;
		auto radiusMin = radius - 2.0f;
		for (auto i = 0; i < 360; i += 3)
		{
			auto degInRad = i*DEG2RAD;
...
void DrawLine(float_t startX, float_t startY, float_t endX, float_t endY, char8_t r, char8_t g, char8_t b, char8_t a)
{
	glColor4ub(r, g, b, a);
	glEnable(GL_LINE_SMOOTH);
