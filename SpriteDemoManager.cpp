...
SpriteDemoManager* SpriteDemoManager::instance = NULL;


SpriteDemoManager *SpriteDemoManager::CreateInstance()
{
	if (instance != NULL)return instance;
	else

		instance = new SpriteDemoManager();
	return instance;
}

void SpriteDemoManager::init(int height, int width)
{
	mHeight = height;
	mWidth = width;
...
void SpriteDemoManager::shutdown()
{
	free(mMoodArray);
	free(mPosArrayX);
	free(mPosArrayY);
...
void SpriteDemoManager::renderSprites()
{
...
void SpriteDemoManager::updateSprites(DWORD milliseconds)
{
...
void SpriteDemoManager::randomizeSprites()
{
	for (int y = 0; y < mHeight; ++y)
	{
		for (int x = 0; x < mWidth; ++x)
...
void SpriteDemoManager::setMood(int x, int y, int mood)
{
	assert(x < mWidth);
	assert(y < mHeight);
	assert(mood < 4);
...
void SpriteDemoManager::setID(int x, int y, int id)
{
	assert(x < mWidth);
	assert(y < mHeight);
	assert(id < 8);
...
void SpriteDemoManager::setPosition(int x, int y, float xPosition, float yPosition)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
float SpriteDemoManager::getXPosition(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
float SpriteDemoManager::getYPosition(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
int	SpriteDemoManager::getMood(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
...
int	SpriteDemoManager::getSpriteID(int x, int y)
{
	assert(x < mWidth);
	assert(y < mHeight);
